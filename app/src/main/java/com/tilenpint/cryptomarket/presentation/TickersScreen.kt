...
import com.tilenpint.cryptomarket.presentation.test.usdSymbol
...
fun TickersScreen(
    ...
    TickersContent(
        ...
        onActionSelectTab = {
            viewModel.onAction(TickersAction.SelectTab(it))
        }
    )
}
...
private fun TickersContent(
    ...
    onActionSelectTab: (CurrencySymbol?) -> Unit,
) {
    Scaffold(
        modifier = modifier
    ) { paddingValues ->
            ...
            PullToRefreshBox(
                ...
                when {
                    !state.filteredTickers.isNullOrEmpty() -> {
                        TickersStateContent(
                            state = state,
                            onActionSelectTab = onActionSelectTab
                        )
                    }
...
@Composable
private fun TickersStateContent(
    state: TickersState,
    onActionSelectTab: (CurrencySymbol?) -> Unit
) {
    Column {
        ScrollableTabRow(selectedTabIndex = state.allTabs.indexOf(state.selectedTab)) {
            state.allTabs.forEach { data ->
                Tab(
                    selected = state.selectedTab == data,
                    onClick = { onActionSelectTab(data) }
                ) {
                    Text((data?.shortName ?: stringResource(R.string.all)).uppercase())
                }
            }
        }
            ...
            Row {
                ...
                Column {
                    Text(
                        ...
                        text = symbol.mainSymbol?.fullName ?: ""
                    )
                    Text(
...
@FullScreenPreview
@Composable
fun TickersContentUsdTabPreview() {
    CryptoMarketTheme {
        TickersContent(
            state = TickersState(
                resultTickers = Result.Success(listOf(btcUsdTest, ethUsdTest)),
                selectedTab = usdSymbol
            ),
            onActionSearchChange = {},
            onActionSearchClear = {},
            onActionForceRefresh = {},
